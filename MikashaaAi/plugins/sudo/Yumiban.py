import requests
import random
from MikashaaAi import app, userbot
...
@app.on_message(filters.command(["mika","mikka"], prefixes=["/","y", "Y"]) & admin_filter)
async def restriction_app(app :app, message):
    reply = message.reply_to_message
    chat_id = message.chat.id
    if len(message.text) < 2:
        return await message.reply(random.choice(mika_text))
